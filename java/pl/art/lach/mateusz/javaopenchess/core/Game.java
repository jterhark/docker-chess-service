...
public class Game extends JPanel implements ComponentListener, MouseListener
{
    ...
    public boolean move(int srcX, int srcY, int destX, int destY){
        Square src = getChessboard().getSquare(srcX, srcY);
        Square dest = getChessboard().getSquare(destX, destY);

//        if(cannotInvokeMoveAction(src)){
//            return false;
//        }
        selectSquare(src);
        try{
            invokeMoveAction(dest);
